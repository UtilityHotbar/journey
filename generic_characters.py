...
class Adventurer(dict):
    def __init__(self, monologue=True, parent=None):
        ...
        for term in range(terms_lived):
            if vocation == 'soldier':
                ...
                sav_list = ['exhaustion', 'hazard']
            life_event = ['wound', 'wound', 'skill', 'save', 'save', 'none', 'promote'][
                min(roll('1d6+' + str(self.get_mod(data['ABL_' + prime_req]))), 6)]
            if life_event == 'wound':
                if monologue:
    ...
    def attack(self, target):
        if target:
            ...
            if roll('1d20') + math.ceil(target.level/5) < roll('1d20') + self.get_mod(self["ABL_dexterity"]) + max(
                    self["SKL_melee"], self["SKL_archery"]):
...
class Party:
    ...
    def remove(self, member, quest_complete=False):
        ...
        if len(self.members) == 0:
            if not quest_complete:
                ...
                delay_print(f'Victory is yours! The quest is complete! Your final score was {self.total_score}')
                exit()
